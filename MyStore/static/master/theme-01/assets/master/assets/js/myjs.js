...
function FormAjaxCKeditor(ev , formId , formBtn, formLBtn, errorId) {  
    var funcForm = $(`#${formId}`);
    ev.preventDefault(); 
    $(`#${formBtn}`).css("display", "none");
    $(`#${formLBtn}`).css("display", "block");
...
function FormAjaxCKeditorModal(ev , formId , formBtn, formLBtn, modalId, errorId) {  
    var funcForm = $(`#${formId}`);  
    $(`#${formBtn}`).css("display", "none");
    $(`#${formLBtn}`).css("display", "block");
    for ( instance in CKEDITOR.instances ) {
...
function FormAjaxModalAccepted(ev , modalId, url, pk, status) {  
    let theURLAccepted = url; 
    $.ajax({
      type: "GET",
      url: theURLAccepted,
      data: { code : pk, part : '1', status:status },
      success: function (data) {
        if (data.type == "success") {
          toastr.success(data.msg); 
          dt.map(item => {
...
function FormAjaxModalRejected(ev , modalId, url, pk, status) {    
    let theURLRejected = url; 
    $.ajax({
      type: "GET",
      url: theURLRejected,
      data: { code : pk, part : '1', status:status },
